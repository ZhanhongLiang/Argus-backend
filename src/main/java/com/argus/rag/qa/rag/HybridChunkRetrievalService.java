...
public class HybridChunkRetrievalService {
    ...
    public RetrievedEvidenceBundle retrieve(Long groupId, String question, int topK) {
        ...
        // 用户问题进来之后, 先利用大模型对用户问题进行查询规划, 也就是需要先调用一次大模型
        /** DIRECT    直接用原问题检索
         REWRITE   改写成更适合检索的语句
         DECOMPOSE 拆成多个子问题分别检索
         GLOBAL    全局遍历文档*/
        ...
        int evidenceIndex = 1;
        // 类簇里面；buildEvidenceWindow()` 按 `[expandedStartChunkIndex, expandedEndChunkIndex]` 范围从缓存中取切片文本，拼接为一段带文件名前缀的完整证据文本：
    ...
    private List<RetrievalCluster> buildClusters(List<RetrievalCandidate> rankedCandidates) {
        ...
        for (RetrievalCandidate candidate : rankedCandidates) { // 同个documentId的切片放在一起
