...
public class StoredObjectDocumentReader implements DocumentReader {
    ...
    public List<Document> get() {
        ...
        // 根据objectKey在对象存储中获得文件流
        // storageService从MinIO对象存储取流，MinioStorageService实现了getObject
        try (InputStream inputStream = storageService.getObject(bucket, objectKey)) {
            String content = parser.parse(inputStream); //文件解析器解析文件流得到内容
            return List.of(buildDocument(content, bucket, objectKey)); // 构建spring AI的Document对象,返回List<Document> 对象
