...
public class DocumentUploadService {
    ...
    public UploadInitResponse initUpload(HttpServletRequest request, UploadInitRequest uploadRequest) {
        NormalizedInitRequest normalizedRequest = validateInitRequest(uploadRequest); // 校验上传文件, 所有字段都校验一遍
        Long groupId = normalizedRequest.groupId(); // 群组ID
        CurrentUserService.CurrentUser currentUser = groupMembershipService.requireGroupOwner(groupId); // 校验当前用户是否是该组的创建人
        DocumentEntity existingDocument = documentMapper.selectByGroupIdAndFileHash(groupId, normalizedRequest.fileHash()); // 查询文件是否有
    ...
    public Long completeUpload(HttpServletRequest request, String uploadId) {
        DocumentUploadSessionEntity session = requireOwnedActiveSession(request, uploadId); // 校验
    ...
    private DocumentEntity persistAndFinalizeUploadedDocument(FinalizedUploadCommand command) {
        ...
        // 异步ETL发布事件，这里发布已经合并完成文件的事件，然后再由事件监听器监听到，触发异步ETL
