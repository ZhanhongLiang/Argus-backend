...
public class AssistantService {
    ...
    public void streamChat(
        ...
        String errorMessage = null;
        try {
            // 先保存用户信息，根据当前用户消息和历史消息的消耗token来判断是否维护历史记忆
            /**
             * 保存用户消息到数据库。
            ...
            /** 该处利用了 streamChat(request, chatRequest, eventEmitter)
             ↓
             把 assistantAgentFacade.streamChat(...) 包成 streamExecutor
             ↓
             streamChat(chatRequest, eventEmitter, streamExecutor)
             ↓
             executeAssistantStreaming(..., streamExecutor)
             ↓
             streamExecutor.execute(deltaConsumer)
             ↓
             assistantAgentFacade.streamChat(..., deltaConsumer)
             ↓
             ReactAgent.stream(...)*/
            ...
            // 返回大模型agent利用知识库检索、hook包装后得到的问答
            reply = executionResult.reply();
            // 保存助手回复, 消息持久等等
            AssistantMessageVO assistantMessage = saveAssistantMessage(
                    currentUser.userId(),
